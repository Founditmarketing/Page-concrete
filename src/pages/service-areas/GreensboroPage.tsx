...
export default function GreensboroPage() {
  const cityCenter: [number, number] = [36.0726, -79.7920];
  const hqCenter: [number, number] = [35.9557, -80.0053];

  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = 'Premium Concrete Services in Greensboro, NC | Page Concrete and Outdoor Services';
  }, []);
    ...
    <main className="min-h-screen bg-slate-50 pb-0">
      ...
      <div className="bg-slate-950 pt-48 md:pt-56 pb-16 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Concrete Contractor in <span className="text-primary">Greensboro, NC</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Providing expert residential driveways, custom stamped patios, and commercial concrete services throughout Greensboro.
          </motion.p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:items-start">
            ...
            >
              <h2 className="text-3xl font-black text-slate-900 uppercase mb-6 flex items-center gap-3">
                ...
                Greensboro Concrete Craftsmanship
              </h2>
              <div className="prose prose-lg text-slate-600 space-y-6">
                <p>
                  Page Concrete and Outdoor Services is proud to serve home and business owners across Greensboro, NC. Whether you are looking to revamp a driveway in Sunset Hills or add a stamped pool deck in Lindley Park or Adams Farm, we bring 30 years of seasoned experience to your project.
                </p>
                <p>
                  Greensboro projects require detailed knowledge of Guilford County soils and site grading. Our crews are equipped to handle complex drainage and soil stabilization near landmarks like the Greensboro Coliseum, Tanger Center, and UNCG. We utilize Greensboro's main thoroughfares—including Wendover Avenue, Battleground Avenue, Friendly Avenue, and the I-840 Urban Loop—to transport materials and equipment directly to your site, ensuring we begin pours during optimal hydration windows.
                </p>

                <h3 className="text-2xl font-bold text-slate-900 mt-8 mb-4">Our Core Concrete Services in Greensboro</h3>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Residential Driveways:</strong> High-PSI concrete mixes reinforced with fiber or rebar, custom-designed to handle vehicles and weather.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Stamped & Decorative Patios:</strong> Premium finishes that look like stone, slate, or wood, adding value to properties near Starmount and Fisher Park.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Commercial Flatwork:</strong> ADA-compliant access ramps, heavy parking pads, and walkways for retail and industrial properties.</span>
                  </li>
                </ul>
            ...
            >
                ...
                {CITIES.map((city) => (
                  <li key={city.id} className="flex items-center gap-3">
                    ...
                    <Link 
                      ...
                      className={`font-bold text-base transition-colors ${city.id === 'Greensboro' ? 'text-primary' : 'text-slate-800 hover:text-primary'}`}
                    >
                      {city.id}
                    </Link>
                  </li>
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                ...
                <p className="text-sm text-slate-500 font-medium mb-4">We provide detailed, free site inspections and written estimates for residential and commercial concrete projects in Greensboro.</p>
                <Link 
                  to="/contact" 
        ...
        >
          ...
          {CITIES.map((city) => (
            <Marker 
              ...
              icon={createIcon(city.id === 'Greensboro')}
            >
              <Popup>
                <div className="font-bold text-slate-900 text-sm mb-1">{city.id}{city.id === 'Greensboro' ? ' (You Are Here)' : ''}</div>
