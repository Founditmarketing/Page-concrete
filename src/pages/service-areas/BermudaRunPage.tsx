...
export default function BermudaRunPage() {
  const cityCenter: [number, number] = [35.9962, -80.4284];
  const hqCenter: [number, number] = [35.9557, -80.0053];

  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = 'Premium Concrete Services in Bermuda Run, NC | Page Concrete and Outdoor Services';
  }, []);
    ...
    <main className="min-h-screen bg-slate-50 pb-0">
      ...
      <div className="bg-slate-950 pt-48 md:pt-56 pb-16 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Concrete Contractor in <span className="text-primary">Bermuda Run, NC</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Delivering high-end residential patios, stamped pool decks, and custom driveways to Davie County’s premier gated community.
          </motion.p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:items-start">
            ...
            >
              <h2 className="text-3xl font-black text-slate-900 uppercase mb-6 flex items-center gap-3">
                ...
                Bermuda Run Concrete Craftsmanship
              </h2>
              <div className="prose prose-lg text-slate-600 space-y-6">
                <p>
                  Bermuda Run, NC represents the pinnacle of residential design in Davie County. From the sprawling golf course homes inside the Bermuda Run Country Club and Kinderton Village to the estates in Bermuda Run West and River Hill, Page Concrete and Outdoor Services offers premium architectural concrete installations.
                </p>
                <p>
                  We are highly experienced with strict local homeowners association (HOA) specifications and town regulations in Bermuda Run. Whether you want to replace a cracked aggregate driveway near WinMock at Kinderton or require a custom-stamped patio adjacent to the Yadkin River, our crews guarantee clean workspaces and neat borders. We travel along I-40, NC-801, and US-158 to deliver prompt, timely service.
                </p>

                <h3 className="text-2xl font-bold text-slate-900 mt-8 mb-4">Our Core Concrete Services in Bermuda Run</h3>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Residential Driveways:</strong> Custom tear-outs, base packing, and structural steel reinforcement to support heavy personal vehicles.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Stamped Patios & Pool Decks:</strong> Multi-layered color sealing with slip-resistant additives, perfectly integrated into golf course backyard landscapes.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Commercial Walks & Curbs:</strong> Clean, durable walkways and handicap-accessible ramps for local business parks.</span>
                  </li>
                </ul>
            ...
            >
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                ...
                <p className="text-sm text-slate-500 font-medium mb-4">We provide detailed, free site inspections and written estimates for residential and commercial concrete projects in Bermuda Run.</p>
                <Link 
                  to="/contact" 
        ...
        >
          ...
          {CITIES.map((city) => (
            <Marker 
              ...
              icon={createIcon(city.id === 'Bermuda Run')}
            >
              <Popup>
                <div className="font-bold text-slate-900 text-sm mb-1">{city.id}{city.id === 'Bermuda Run' ? ' (You Are Here)' : ''}</div>
