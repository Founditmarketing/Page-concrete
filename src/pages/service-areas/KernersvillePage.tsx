...
export default function KernersvillePage() {
  const cityCenter: [number, number] = [36.1199, -80.0736];
  const hqCenter: [number, number] = [35.9557, -80.0053];

  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = 'Premium Concrete Services in Kernersville, NC | Page Concrete and Outdoor Services';
  }, []);
    ...
    <main className="min-h-screen bg-slate-50 pb-0">
      ...
      <div className="bg-slate-950 pt-48 md:pt-56 pb-16 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Concrete Contractor in <span className="text-primary">Kernersville, NC</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Providing premium residential driveways, decorative stamped patios, and commercial concrete services in the Heart of the Triad.
          </motion.p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:items-start">
            ...
            >
              <h2 className="text-3xl font-black text-slate-900 uppercase mb-6 flex items-center gap-3">
                ...
                Kernersville Concrete Craftsmanship
              </h2>
              <div className="prose prose-lg text-slate-600 space-y-6">
                <p>
                  Kernersville’s geographic location makes it the true heart of the Piedmont Triad. From our central dispatch, Page Concrete and Outdoor Services offers rapid response times to homeowners and businesses throughout Kernersville, NC, providing premium flatwork engineered to outlast the competition.
                </p>
                <p>
                  Our crews know the local layout of Kernersville, working in neighborhoods such as Century Park, Gralyn, Abbots Creek, and near Bunker Hill. Whether you are seeking a stamped stone walkway near Korner’s Folly and the Paul J. Ciener Botanical Garden, or a heavy-duty parking expansion off NC Highway 66, NC Highway 150, or Macy Grove Road, we plan and execute all pours with precision. We ensure proper subgrade prep and layout control joints to keep your concrete crack-free.
                </p>

                <h3 className="text-2xl font-bold text-slate-900 mt-8 mb-4">Our Core Concrete Services in Kernersville</h3>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Residential Driveways:</strong> Custom tear-outs and fresh pours with optimal subgrade packing and rebar to prevent cracking under loads.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Stamped & Decorative Concrete:</strong> Turn your backyard or entryway near Triad Park into an elegant stone-look feature.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Commercial Walkways & Slabs:</strong> ADA-compliant ramps, durable concrete walkways, and warehouse slabs.</span>
                  </li>
                </ul>
            ...
            >
                ...
                {CITIES.map((city) => (
                  <li key={city.id} className="flex items-center gap-3">
                    ...
                    <Link 
                      ...
                      className={`font-bold text-base transition-colors ${city.id === 'Kernersville' ? 'text-primary' : 'text-slate-800 hover:text-primary'}`}
                    >
                      {city.id}
                    </Link>
                  </li>
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                ...
                <p className="text-sm text-slate-500 font-medium mb-4">We provide detailed, free site inspections and written estimates for residential and commercial concrete projects in Kernersville.</p>
                <Link 
                  to="/contact" 
        ...
        >
          ...
          {CITIES.map((city) => (
            <Marker 
              ...
              icon={createIcon(city.id === 'Kernersville')}
            >
              <Popup>
                <div className="font-bold text-slate-900 text-sm mb-1">{city.id}{city.id === 'Kernersville' ? ' (You Are Here)' : ''}</div>
