...
export default function ClemmonsPage() {
  const cityCenter: [number, number] = [36.0232, -80.3820];
  const hqCenter: [number, number] = [35.9557, -80.0053];

  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = 'Premium Concrete Services in Clemmons, NC | Page Concrete and Outdoor Services';
  }, []);
    ...
    <main className="min-h-screen bg-slate-50 pb-0">
      ...
      <div className="bg-slate-950 pt-48 md:pt-56 pb-16 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Concrete Contractor in <span className="text-primary">Clemmons, NC</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Providing premium residential driveways, decorative stamped patios, and commercial concrete services in Clemmons.
          </motion.p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:items-start">
            ...
            >
              <h2 className="text-3xl font-black text-slate-900 uppercase mb-6 flex items-center gap-3">
                ...
                Clemmons Concrete Craftsmanship
              </h2>
              <div className="prose prose-lg text-slate-600 space-y-6">
                <p>
                  Clemmons, NC is one of the most rapidly growing residential areas in Forsyth County. Page Concrete and Outdoor Services is proud to serve local homeowners in beautiful neighborhoods like Waterford, Meadowbrook, Salem Glen, and Asbury Place, providing long-lasting concrete solutions that increase curb appeal.
                </p>
                <p>
                  From building decorative pool decks and custom stamped patios near Tanglewood Park (home of the annual Festival of Lights and pristine golf courses) to pouring structural slabs close to the Yadkin River, we understand local soil dampness and compaction requirements. We use heavy machinery and skilled crews, dispatching easily along US-158 (Clemmons Road), Harper Road, Clemmonsville Road, and I-40 to handle your job efficiently.
                </p>

                <h3 className="text-2xl font-bold text-slate-900 mt-8 mb-4">Our Core Concrete Services in Clemmons</h3>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Residential Driveways:</strong> Custom-graded and packed driveways with rebar grids, built to withstand local freeze-thaw cycles.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Stamped & Decorative Patios:</strong> Premium stamped finishes resembling natural slate, flagstone, or brick for outdoor living.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Commercial Walks & Steps:</strong> Heavy-duty parking spaces, dumpster pads, and ADA-compliant access paths.</span>
                  </li>
                </ul>
            ...
            >
                ...
                {CITIES.map((city) => (
                  <li key={city.id} className="flex items-center gap-3">
                    ...
                    <Link 
                      ...
                      className={`font-bold text-base transition-colors ${city.id === 'Clemmons' ? 'text-primary' : 'text-slate-800 hover:text-primary'}`}
                    >
                      {city.id}
                    </Link>
                  </li>
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                ...
                <p className="text-sm text-slate-500 font-medium mb-4">We provide detailed, free site inspections and written estimates for residential and commercial concrete projects in Clemmons.</p>
                <Link 
                  to="/contact" 
        ...
        >
          ...
          {CITIES.map((city) => (
            <Marker 
              ...
              icon={createIcon(city.id === 'Clemmons')}
            >
              <Popup>
                <div className="font-bold text-slate-900 text-sm mb-1">{city.id}{city.id === 'Clemmons' ? ' (You Are Here)' : ''}</div>
