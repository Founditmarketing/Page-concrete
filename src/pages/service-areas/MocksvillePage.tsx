...
export default function MocksvillePage() {
  const cityCenter: [number, number] = [35.8943, -80.5615];
  const hqCenter: [number, number] = [35.9557, -80.0053];

  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = 'Premium Concrete Services in Mocksville, NC | Page Concrete and Outdoor Services';
  }, []);
    ...
    <main className="min-h-screen bg-slate-50 pb-0">
      ...
      <div className="bg-slate-950 pt-48 md:pt-56 pb-16 px-4 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center relative z-10">
          ...
          >
            Concrete Contractor in <span className="text-primary">Mocksville, NC</span>
          </motion.h1>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            Delivering high-quality residential driveways, decorative stamped patios, and commercial concrete services in Mocksville.
          </motion.p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:items-start">
            ...
            >
              <h2 className="text-3xl font-black text-slate-900 uppercase mb-6 flex items-center gap-3">
                ...
                Mocksville Concrete Craftsmanship
              </h2>
              <div className="prose prose-lg text-slate-600 space-y-6">
                <p>
                  Mocksville, NC is the county seat of Davie County, rich with historical charm. Page Concrete and Outdoor Services is proud to provide professional concrete flatwork and decorative designs that complement local architectural styles and properties.
                </p>
                <p>
                  Our crews are active in neighborhoods near Willow Pond, Milling Road, and Oak Hills. Whether you are looking to install a new decorative walkway near Rich Park and the historic Davie County Courthouse square, or require a heavy-duty slab replacement off US-64, US-601, NC-801, or I-40, we guarantee clean joints and structural durability.
                </p>

                <h3 className="text-2xl font-bold text-slate-900 mt-8 mb-4">Our Core Concrete Services in Mocksville</h3>
                <ul className="space-y-3">
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Residential Driveways:</strong> Deep-tear outs and pours using high-PSI concrete with wire mesh or rebar reinforcement.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Stamped & Decorative Concrete:</strong> Turn your backyard into a retreat with slate, flagstone, or brick-stamped patios.</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <CheckCircle2 className="text-[#1d4e89] shrink-0 mt-1" size={20} />
                    <span><strong>Commercial Walkways & Steps:</strong> Heavy-duty parking expansions, dumpster pads, and ADA-compliant access ramps.</span>
                  </li>
                </ul>
            ...
            >
                ...
                {CITIES.map((city) => (
                  <li key={city.id} className="flex items-center gap-3">
                    ...
                    <Link 
                      ...
                      className={`font-bold text-base transition-colors ${city.id === 'Mocksville' ? 'text-primary' : 'text-slate-800 hover:text-primary'}`}
                    >
                      {city.id}
                    </Link>
                  </li>
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                ...
                <p className="text-sm text-slate-500 font-medium mb-4">We provide detailed, free site inspections and written estimates for residential and commercial concrete projects in Mocksville.</p>
                <Link 
                  to="/contact" 
        ...
        >
          ...
          {CITIES.map((city) => (
            <Marker 
              ...
              icon={createIcon(city.id === 'Mocksville')}
            >
              <Popup>
                <div className="font-bold text-slate-900 text-sm mb-1">{city.id}{city.id === 'Mocksville' ? ' (You Are Here)' : ''}</div>
